...
func routesProtoToSlice(routes []*v3routepb.Route, csps map[string]clusterspecifier.BalancerConfig) ([]*Route, map[string]bool, error) {
	...
	for _, r := range routes {
		...
		case *v3routepb.Route_Route:
			...
			fmt.Printf("DEBUGR0 printer routesProtoToSlice route.HostRewriteLiteral '%s'\n", route.HostRewriteLiteral)
			logger.Errorf("DEBUGR0 logger routesProtoToSlice route.HostRewriteLiteral '%s'\n", route.HostRewriteLiteral)
